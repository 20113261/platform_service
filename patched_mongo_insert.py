#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2017/11/2 下午4:31
# @Author  : Hou Rong
# @Site    : 
# @File    : patched_mongo_insert.py
# @Software: PyCharm
import collections
import datetime
import warnings
...
class Collection(common.BaseObject):
    ...
    def _insert(self, sock_info, docs, ordered=True, check_keys=True,
        ...
        if sock_info.max_wire_version > 1 and acknowledged:
            ...
            final_result = {'n': 0, 'err': 0}
            for _, each in results:
                n = each.get('n', 0)
                write_errors = each.get('writeErrors', [])

                final_result['n'] += n
                final_result['err'] += len(write_errors)
