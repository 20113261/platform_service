...
class Last_Google(BaseSDK):
    def _execute(self, **kwargs):
        ...
        insert_sql = "insert into google_hotel6(hotel_name,agoda,booking,ctrip,elong,expedia,hotels,other_source) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"
...
if __name__ == "__main__":
    ...
    google_sql = "select distinct(hotel_name),hotel_name_en from base_data.hotel limit 1000"
    conn = pymysql.connect(**google_config)
    cursor = conn.cursor()
    cursor.execute(google_sql)
    google_values = cursor.fetchall()
    for i,j in google_values:
        hotel_name = j if j else i
        args = {
            'hotel_name': hotel_name,
            'source': 'google',
            'spider_tag': 'googleListHotel',
        }
