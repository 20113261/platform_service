    ...
    def _execute(self, **kwargs):
        ...
        if source == 'daodao':
            for hotel in hotel_result:
                ...
                temp_save.append((name, name_en, 'daodao', 1, json.dumps(hotel) if hotel else None))

        elif source == 'google':
            for hotel in hotel_result:
                status = 1 if hotel_result else 0
                temp_save.append((name, name_en, 'google', status, json.dumps(hotel) if hotel else None))
        ...
        def hotel_list_insert_db(table_name, res_data):
            try:
                ...
                sql = "insert into {}(name, name_en, `from`, status, source_list) VALUES (%s,%s,%s,%s,%s)".format(table_name)
