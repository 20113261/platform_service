...
def ctrip_parser(page, url, other_info):
    ...
    try:
        ...
        for item in items:
            item = item.lower()
            if "客房wifi" in item:
                hotel.facility_content['Room_wifi'] = item
            elif "客房有线网络" in item:
                hotel.facility_content['Room_wired'] = item
            elif "公" and "WiFi".lower() in item:
                hotel.facility_content['Public_wifi'] = item
            elif "公" and "有线网络" in item:
                hotel.facility_content['Public_wired'] = item
            elif "停车场" in item:
                hotel.facility_content['Parking'] = item
            elif "机场班车" in item:
                hotel.facility_content['Airport_bus'] = item
            elif "代客泊车" in item:
                hotel.facility_content['Valet_Parking'] = item
            elif "叫车服务" in item:
                hotel.facility_content['Call_service'] = item
            elif "租车服务" in item:
                hotel.facility_content['Rental_service'] = item
            elif "游泳池" in item:
                hotel.facility_content['Swimming_Pool'] = item
            elif "健身" in item:
                hotel.facility_content['gym'] = item
            elif "SPA".lower() in item:
                hotel.facility_content['SPA'] = item
            elif "酒吧" in item:
                hotel.facility_content['Bar'] = item
            elif "咖啡厅" in item:
                hotel.facility_content['Coffee_house'] = item
            elif "网球场" in item:
                hotel.facility_content['Tennis_court'] = item
            elif "高尔夫球场" in item:
                hotel.facility_content['Golf_Course'] = item
            elif "桑拿" in item:
                hotel.facility_content['Sauna'] = item
            elif "水疗中心" in item:
                hotel.facility_content['Mandara_Spa'] = item
            elif "儿童娱乐场" in item:
                hotel.facility_content['Recreation'] = item
            elif "商务中心" in item:
                hotel.facility_content['Business_Centre'] = item
            elif "行政酒廊" in item:
                hotel.facility_content['Lounge'] = item
            elif "婚礼礼堂" in item:
                hotel.facility_content['Wedding_hall'] = item
            elif "餐厅" in item:
                hotel.facility_content['Restaurant'] = item
            elif "行李寄存" in item:
                hotel.service_content['Luggage_Deposit'] = item
            elif "24小时前台" in item:
                hotel.service_content['front_desk'] = item
            elif "24小时大堂经理" in item:
                hotel.service_content['Lobby_Manager'] = item
            elif "24小时办理入住" in item:
                hotel.service_content['24Check_in'] = item
            elif "24小时安保" in item:
                hotel.service_content['Security'] = item
            elif "礼宾服务" in item:
                hotel.service_content['Protocol'] = item
            elif "叫醒服务" in item:
                hotel.service_content['wake'] = item
            elif "中文前台" in item:
                hotel.service_content['Chinese_front'] = item
            elif "邮政服务" in item:
                hotel.service_content['Postal_Service'] = item
            elif "传真/复印" in item:
                hotel.service_content['Fax_copy'] = item
            elif "洗衣服务" in item:
                hotel.service_content['Laundry'] = item
            elif "擦鞋服务" in item:
                hotel.service_content['polish_shoes'] = item
            elif "前台保险柜" in item:
                hotel.service_content['Frontdesk_safe'] = item
            elif "快速办理入住/退房" in item:
                hotel.service_content['fast_checkin'] = item
            elif "自动柜员机(ATM)/银行服务" in item:
                hotel.service_content['ATM'] = item
            elif "儿童看护服务" in item:
                hotel.service_content['child_care'] = item
            elif "送餐服务" in item:
                hotel.service_content['Food_delivery'] = item
    ...
    try:
        ...
        if fea_params:
            ...
            if "华人礼遇" in fea_name:
                hotel.feature_content['China_Friendly'] = " ".join(fea_service_list)
            elif "浪漫情侣" in fea_name:
                hotel.feature_content['Romantic_lovers'] = " ".join(fea_service_list)
            elif "亲子时光" in fea_name:
                hotel.feature_content['Parent_child'] = " ".join(fea_service_list)
            elif "海滨风光" in fea_name:
                hotel.feature_content['Beach_Scene'] = " ".join(fea_service_list)
            elif "温泉酒店" in fea_name:
                hotel.feature_content['Hot_spring'] = " ".join(fea_service_list)
            elif "日式旅馆" in fea_name:
                hotel.feature_content['Japanese_Hotel'] = " ".join(fea_service_list)
            elif "休闲度假" in fea_name:
                hotel.feature_content['Vacation'] = " ".join(fea_service_list)
        if fea_params2:
            for fea in fea_params2:
                if "华人礼遇" in fea:
                    hotel.feature_content['China_Friendly'] = fea
                elif "浪漫情侣" in fea:
                    hotel.feature_content['Romantic_lovers'] = fea
                elif "亲子酒店" in fea:
                    hotel.feature_content['Parent_child'] = fea
                elif "海滨风光" in fea:
                    hotel.feature_content['Beach_Scene'] = fea
                elif "温泉酒店" in fea:
                    hotel.feature_content['Hot_spring'] = fea
                elif "日式旅馆" in fea:
                    hotel.feature_content['Japanese_Hotel'] = fea
                elif "休闲度假" in fea:
                    hotel.feature_content['Vacation'] = fea
