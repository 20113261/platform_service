...
class CsairRoundSpider(Spider):
    ...
    def __init__(self, task=None):
        super(CsairRoundSpider, self).__init__(task=task)
        self.task = task
        self.task_info = {}
        self.id_list = []
        self.result_row = {}
    ...
    def getdata(self):
        dep, arr, date1,date2 = self.task.content.split("&")
        date1 = date1[:4]+"-"+date1[4:6]+"-"+date1[6:]
        date2 = date2[:4]+"-"+date2[4:6]+"-"+date2[6:]
        data = "language=zh&country=cn&m=1&flexible=1&adt=1&cnn=0&inf=0&" \
               "dep={}&arr={}&date={}&dep={}&arr={}&date={}".format(dep,arr,date1,arr,dep,date2,)
        return data
    ...
    def targets_request(self):
        ...
        def get_flight():
            ...
            for id_ in self.id_list:
                ...
                if len(round_Fligh) < 8:
                    self.count += 1
                    round_Fligh.append(req)
                else:
                    break
...
if __name__ == '__main__':
    ...
    task = Task(source='csairRoundFlight', content='PEK&PAR&20180309&201803011')
    spider.task = task
    print spider.crawl()
    print spider.result
