...
from mioji.common.spider import Spider, request, PROXY_NONE, PROXY_REQ, PROXY_FLLOW
from igola_api import Handler
from mioji.common.logger import logger
from mioji.common.check_book.check_book_ratio import use_record_qid


class igolaMultiFlightSpider(Spider):
    source_type = 'igola'
    # 基础数据城市酒店列表 & 例行城市酒店
    targets = {
    ...
    #   对应多个原爬虫
    old_spider_tag = {
        'igolaMultiFlight': {'required': ['MultiFlight']}
    }
...
# uat account
    auth = {
        'partnerId': 'miojiuat',
        'appid': 'miojiuat',
        'appsecurity': 'ad7fd9a2-34b6-446a-854c-a041ad1c220c',
        'host': "http://112.74.38.98:9114",
    }
    task.ticket_info = {
        'v_seat_type': 'E',
    ...
    task.content = 'ZAG&DXB&20171220|DXB&HKG&20171222'
    task.redis_key = 'default_redis_key'
    spider = igolaMultiFlightSpider()
    spider.task = task
