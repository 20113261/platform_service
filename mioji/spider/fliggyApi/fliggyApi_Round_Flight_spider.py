...
class FliggyApiRoundSpider(Spider):
    source_type = 'fliggyApiRoundFlight'
    targets = {'Flight': {'version': 'InsertRoundFlight2'}}
    old_spider_tag = {'fliggyApiRoundFlight': {'required': ['Flight']}}
    ...
    def parse_content(self):
        ...
        dest_time = "{}-{}-{}".format(content[3][:4], content[3][4:6], content[3][6:8])
        return dept_code, dest_code, dept_time, dest_time

    def filter_attr(self, switch, all_ticket):
        tickets = []
        if switch:
            for i in range(len(all_ticket)):
                if str(bin(json.loads(all_ticket[i][45])['product_attr']))[-1] == "1":
    ...
    def targets_request(self):
        ...
        dept_code, dest_code, dept_time, dest_time = self.parse_content()
        try:
        ...
        od_info = [
            ...
            {
                "dep_city_code": str(dest_code),
                "arr_city_code": str(dept_code),
                "dep_date": dest_time
            }
        ]
...
if __name__ == '__main__':
    ...
    task.content = "PVG&MFM&20180421&20180423"
    task.other_info = {}
    ...
    spider = FliggyApiRoundSpider(task)
    spider.task = task
