...
class hiltonHotelSpider(Spider):
    ...
    def targets_request(self):
        ...
        @request(retry_count=5, proxy_type=PROXY_REQ, store_page_name="price_list_page_{}_{}".format(tid, used_times))
        def first_page():
            return {
        ...
        for pro_list in [first_heap[:15], first_heap[15:]]:
            if self.ceil:
                yield other_page
                @request(retry_count=0, proxy_type=PROXY_FLLOW, binding=[self.parse_room], async=True, store_page_name="price_detail_{}_{}".format(tid, used_times))
                def detail():
                    return pro_list
                yield detail
                self.ceil = False
...
if __name__ == '__main__':
    ...
    task = Task()
    task.content = "NULL&50012&NYCDTDT&2&20180121"
