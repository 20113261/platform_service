...
class StarWoodSpider(Spider):
    ...
    def parse_service(self, resp):
        ...
        if 'internet access' in has_other.lower():
            ...
            if 'wired & wireless high speed internet access in guest rooms' in wifi_info:
                self.hotel.facility_content['Room_wifi'] = \
                    self.hotel.facility_content['Room_wired'] = 'Wired & Wireless High Speed Internet Access in Guest Rooms'
            if 'Wireless High Speed Internet Access'.lower() in wifi_info:
                self.hotel.facility_content['Room_wifi'] = 'Wireless High Speed Internet Access'
            if 'Wired High Speed Internet Access'.lower() in wifi_info:
                self.hotel.facility_content['Room_wired'] = 'Wired High Speed Internet Access'
            if 'Wireless High Speed Internet Access in Public Areas'.lower() in wifi_info:
                self.hotel.facility_content['Pubic_wifi'] = 'Wireless High Speed Internet Access in Public Areas'
            if 'Wired High Speed Internet Access in Public Areas'.lower() in wifi_info:
                self.hotel.facility_content['Public_wired'] = 'Wired High Speed Internet Access in Public Areas'
        ...
        for content in content_list:
            ...
            if 'Self'.lower() in content_lower and 'Parking'.lower() in content_lower:
                self.hotel.facility_content['Parking'] = content
            if 'Free'.lower() in content_lower and 'Airport'.lower() in content_lower:
                self.hotel.facility_content['Airport_bus'] = content

            if 'Pool'.lower() in content_lower:
                a += 1
                if a > 2:
                    self.hotel.facility_content['Swimming_Pool'] += content
                else:
                    self.hotel.facility_content['Swimming_Pool'] = content

            if 'WestinWORKOUT® Fitness Studio'.lower() in content_lower:
                self.hotel.facility_content['gym'] = content
            if 'Spa'.lower() in content_lower:
                self.hotel.facility_content['SPA'] = content
            if 'Bar'.lower() in content_lower:
                self.hotel.facility_content['Bar'] = content
            if 'Spa'.lower() in content_lower:
                self.hotel.facility_content['Mandara_Spa'] = content
            if 'kids' in content_lower and 'club' in content_lower:
                self.hotel.facility_content['Recreation'] = content
            if 'Business Center'.lower() in content_lower:
                self.hotel.facility_content['Business_Centre'] = content
            self.hotel.facility_content['Restaurant'] = 'Restaurant'

            if 'Concierge Desk'.lower() in content_lower:
                self.hotel.service_content['Protocol'] = content
            if 'Copy'.lower() in content_lower or 'faxing' in content_lower:
                self.hotel.service_content['Fax_copy'] = content

            if 'Shoe Shine Service'.lower() in content_lower:
                self.hotel.service_content['polish_shoes'] = content

            if 'Laundry'.lower() in content_lower:
                self.hotel.service_content['Laundry'] = content

            if 'Luggage'.lower() in content_lower:
                self.hotel.service_content['Luggage_Deposit'] = content
    ...
    def parse_hotel(self, req, resp):
        ...
        self.hotel.others_info = json.dumps({})
        # res = json.dumps(self.hotel.to_dict())
        # print res
