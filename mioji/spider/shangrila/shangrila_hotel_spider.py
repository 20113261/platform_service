...
class ShangRiLaDetailSpider(Spider):
                    ...
                    for service in service_all:
                        for fac_value in facilities_dict.values():
                            if fac_value in service:
                                ...
                                hotel2.facility_content[reverse_facility_dict[fac_value]] = service
                        for sev_value in service_dict.values():
                            if sev_value in service:
                                service = self.clean_data(service)
                                hotel2.service_content[reverse_sevice_dict[sev_value]] = service
                    self.service = hotel2.service_content
                    self.facility = hotel2.facility_content
                except Exception as e:
                    self.service = "NULL"
                    self.facility = "NULL"
    ...
    def parse_hotel(self, req, resp):
        ...
        hotel.facility = str(self.facility)
        hotel.service = str(self.service)
        ...
        if 'meta_review' in req_url or 'seal' in req_url or 'partnerId' in req_url:
            ...
            # res = json.loads(hotel)
            result.append(hotel.__dict__)
            return result
