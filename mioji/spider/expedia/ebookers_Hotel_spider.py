...
class ebookersHotelSpider(BaseHotelSpider):
    source_type = 'ebookersHotel'
    targets = {
        'hotel': {},
    ...
    def setting(self):
        ...
        self.check_in_new = self.check_in[8:10] + "%2F" + self.check_in[5:7] + '%2F' + self.check_in[0:4]
        self.check_out = str(self.date1 + datetime.timedelta(self.dur))[0:10]
        self.check_out_new = self.check_out[8:10] + "%2F" + self.check_out[5:7] + '%2F' + self.check_out[0:4]
        self.children = 0
        ...
        self.json_url = 'https://www.ebookers.com/api/infosite/{0}/getOffers?token={1}&brandId={2}&isVip=false&chid=&chkin={3}&chkout={4}&ts={5}{6}'
        self.cid = self.task.ticket_info.get('cid', None)
        self.urltmp = task_list[0] + 'hkin={0}&chkout={1}'.format(self.check_in_new, self.check_out_new)
...
if __name__ == '__main__':
    ...
    task.content = 'https://www.ebookers.comb''?&2&20180107'
    spider = ebookersHotelSpider()
    spider.task = task
    print spider.crawl()
    print spider.result