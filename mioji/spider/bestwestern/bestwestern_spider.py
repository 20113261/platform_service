...
class RoomSpider(Spider):
                    ...
                    'req': {
                        ...
                        'headers': {
                            'Cookie': "CURRENCY-track={CURRENCY-track};"
                                      ...
                                      "search_distance={search_distance};search_numAdults={search_numAdults}".
                                format(**self.cookies),
    ...
    def parse_Room(self, req, resp):
        ...
        for r in self.base_room_info_list:
            if r['source_roomid'] == source_roomid:
                ...
                # 房间类型
                room.room_type = html.xpath(
                    '//div[@class="paymentSummaryFixedWrapper"]//p[contains(@class,"bedsAdultChildrenLabel")]/text()')[
                    0].split(",")[0].strip()
                # 真实购买源
                ...
                # 房间描述
                room.room_desc = base_info['room_desc'].strip() + "," + html.xpath(
                    '//div[contains(@class,"roomDetailsContentContainer")]//div[contains(@class,"paddingContainer")][1]/text()')[
                    0].strip()
                # 酒店url
                ...
                self.rooms_info.append(room_info)
        print self.rooms_info
        # 这个rooms_info是截止到当前时间的所有请求返回的所有结果，所以只需要取最后一个请求返回的self.rooms_info就可以了
    ...
    def process_task(self):
        ...
        # 每间房入住人数列表
        occ_list = self.task.ticket_info['room_info']
        # 房间数
        room_count = len(self.task.ticket_info['room_info'])

        self.task_info = dict(
            search_numAdults=self.__get_num_adults_str(occ_list),
            search_numChild=self.__get_num_child_str(occ_list),
    ...
    # {'adult_info': [33, ], 'child_info': [7, 12, ]}
    # ]
    def __get_num_adults_str(self, occ_list):
        adults = list()
        for occ in occ_list:
            adults.append(str(len(occ['adult_info'])))
        adults_str = ",".join(adults)  # a = '1,1,1'
        a = urllib.quote(adults_str)  # b = '1%2C1%2C1'
        return a

    # 得到房间儿童数字符串
    def __get_num_child_str(self, occ_list):
        children = list()
        for occ in occ_list:
            children.append(str(len(occ['child_info'])))
        children_str = ",".join(children)  # a = '1,1,1'
        c = urllib.quote(children_str)  # b = '1%2C1%2C1'
        return c
    ...
    def __get_occ(self, resp):
        ...
        m = re.search(p, resp)
        num = m.group(1)
        for room_info in self.task.ticket_info['room_info']:
            if int(num) >= (len(room_info['adult_info']) + len(room_info['child_info'])):
                continue
            else:
                raise parser_except.ParserException(29, "无房间")
        return num
...
if __name__ == '__main__':
    ...
    # task.content = "best-western-hotel-am-spittelmarkt-berlin-95382&1&20180420"
    task.content = "best-western-london-highbury-london-83939&1&20180420"
    task.ticket_info = {
        'room_info': [
            {'adult_info': [33, 44, 55], 'child_info': [7, 12, ]},
            {'adult_info': [33, ], 'child_info': [7, 12, ]}
