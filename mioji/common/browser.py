...
REQ_PARAMS = ['method', 'url', 'params', 'data', 'headers', 'cookies', 'files',
...
class MechanizeCrawler(object):
    def __init__(self, referer='', headers=None, p='', md5='', qid='', **kw):
        ...
        headers = {'User-agent': random_useragent(), 'Connection': 'Close'}
    ...
    def req(self, url, method='get', params=None, data=None, json=None, timeout=(6, None), verify=False, **kw):
        for k in kw.keys():
            if k not in REQ_PARAMS:
                logger.warning(current_log_tag() + '[出现不能解析的 req 请求参数][{0}]'.format(k))

        new_kw = {k: v for k, v in kw.items() if k in REQ_PARAMS}
