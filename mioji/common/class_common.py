...
class Flight:
    ...
    def __cmp__(self, other):
        if self.fly_seconds > other.fly_seconds:
            return 1
        elif self.fly_seconds < other.fly_seconds:
            return -1
        else:
            return 0
...
class Hotel_New:
    ...
    def to_dict(self):
        ...
        result["facility_content"] = facility
        result["feature_content"] = feature
        result["service_content"] = service
        return dumps(result)

class AgeInfo(object):
    def __init__(self, v_age=-1, age_range=[2, 12]):
        """-1是本地测试用的，年龄范围默认是2岁和12岁"""
    ...
    def get_age_list(self):
        ...
        if self.v_age == -1:
            return {'Adult': [18], 'Child': [5], 'Infant': [2]}

        result = {'Adult': [], 'Child': [], 'Infant': []}

        for age in self.v_age:
            if age <= self.age_range[0]:
                result['Infant'].append(age)
            elif age <= self.age_range[1]:
                result['Child'].append(age)
            else:
                result['Adult'].append(age)

        return result

    def get_age_count_dic(self):
        """获取每个年龄段的个数"""

        age_count = self.get_age_list()
    ...
    def get_age_count(self):
        ...
        age_count = self.get_age_count_dic()
        return age_count['Adult'], age_count['Child'], age_count['Infant']
