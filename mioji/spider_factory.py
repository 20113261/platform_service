...
class LoadSpiderException(Exception):
    pass
...
class SpiderFactory(object):
    ...
    def __load(self):
        ...
        for source in source_module_names:
            exist_source_spider = set([])
            exist_old_spider = set([])
            ...
            for spider_module in spider_module_names:
                try:
                    ...
                    if spider_module.endswith('_spider'):
                        ...
                        if desc:
                            # 不能存在相同的 source_type
                            source_type = desc[0]['source_type']
                            # if source_type in exist_source_spider:
                            #     raise LoadSpiderException('init spider error source:{0} 存在相同的source_type:{1}, class={2},'
                            #                               '告诉我用哪个...'.format(
                            #         source, source_type, desc[0]['spider_class'])
                            #     )
                            # exist_source_spider.add(source_type)

                            spider_list[source][source_type] = desc[0]

                        if old_tag:
                            for k, v in old_tag[0].items():
                                # 不能存在相同的 old_spider_tag
                                if k in exist_old_spider:
                                    msg = 'init spider error source:{0} old_spider_tag:{1}, class={2}; exist={3}告诉我用哪个...'.format(
                                            source, k, v['spider_class'], old_source_spider[k])
                                    logger.warn(msg)
                                    # raise LoadSpiderException(msg)

                                exist_old_spider.add(k)
                                old_source_spider[k] = v
                except LoadSpiderException as e:
                    raise e
                except Exception:
